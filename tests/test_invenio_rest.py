...
def test_cors(app):
    ...
    with app.test_client() as client:
        ...
        assert len(res.headers["Access-Control-Expose-Headers"].split(",")) == len(
...
def test_content_negotiation_method_view_global_serializers(app):
    ...
    _subtest_content_negotiation_method_view(
        app,
        _ObjectItem,
        params={
            "serializers": {
...
def test_get_method_serializers(app):
    ...
    # Default media type
    assert v.get_method_serializers("GET")[1] == v.default_method_media_type["GET"]
    assert v.get_method_serializers("HEAD")[1] == v.default_method_media_type["GET"]
    assert v.get_method_serializers("POST")[1] == v.default_method_media_type["POST"]
    assert v.get_method_serializers("PUT")[1] == v.default_media_type

    assert v.get_method_serializers("DELETE")[0] == v.method_serializers["DELETE"]
...
def test_match_serializers_headers(app):
    ...
    v = ContentNegotiatedMethodView(
        method_serializers=dict(
            ...
            GET={"application/json": "json-get", "application/marcxml+xml": "xml-get"},
            POST={
    ...
    tests = [
        ...
        ("application/json, application/marcxml+xml; q=0.5", "GET", "json-get"),
        ("application/json; q=0.4, application/marcxml+xml; q=0.6", "GET", "xml-get"),
        ("application/marcxml+xml; q=0.6, application/json; q=0.4", "GET", "xml-get"),
        # Test delete
        (
            "application/marcxml+xml; q=0.6, application/json; q=0.4",
            "DELETE",
            "any-delete",
        ),
    ...
    for accept, method, expected_serializer in tests:
        ...
        _test_march_serializers(app, v, params, method, accept, expected_serializer)
...
def test_match_serializers_query_arg(app):
    ...
    tests = [
        ...
        (urlencode({arg_name: "json"}), "application/marcxml+xml", "json-get"),
        # Should serialize to json
    ...
    for arg, accept, expected_serializer in tests:
        params = dict(headers=[("Accept", accept)], query_string=arg) if accept else {}
        _test_march_serializers(app, v, params, "GET", accept, expected_serializer)
...
def _test_march_serializers(
    ...
    with app.test_request_context(**request_params):
        assert (
            view_method.match_serializers(
                *view_method.get_method_serializers(request_method)
            )
            == expected_serializer
        ), "Accept: {0}, Method: {1}".format(request_accept_header, request_method)


def _subtest_content_negotiation_method_view(app, content_negotiated_class, params):
    """Test a given ContentNegotiatedMethodView subclass."""
    app.add_url_rule(
        "/objects/<int:id>",
        view_func=content_negotiated_class.as_view(
            content_negotiated_class.view_name, **params
        ),
    )
    ...
    with app.test_client() as client:
        ...
        def check_normal_response(res, method):
            ...
            assert unquote_etag(res.headers["ETag"]) == unquote_etag(quote_etag("abc"))
        ...
        headers_nonmatch_match = [
            ("Accept", "application/json"),
            ("If-None-Match", '"xyz", "abc"'),
        ]
        headers_nonmatch_star = [
            ("Accept", "application/json"),
            ("If-None-Match", '"xyz", "*"'),
        ]
        ...
        headers = [("Accept", "application/json"), ("If-None-Match", '"xyz", "def"')]
        for method in all_methods:
            res = method("/objects/1", headers=headers)
            check_normal_response(res, method)
        ...
        headers = [("Accept", "application/json"), ("If-Match", 'W/"abc", W/"def"')]
        query_string_weak_etags = {"weak_etags": True}
        for method in all_methods:
            res = method(
                "/objects/1", headers=headers, query_string=query_string_weak_etags
        ...
        headers = [("Accept", "application/json"), ("If-Match", '"xyz", "def"')]
        for method in all_methods:
            res = method("/objects/1", headers=headers)
            assert res.status_code == 412

        # check non matching If-Match with weak ETags
        headers = [("Accept", "application/json"), ("If-Match", 'W/"xyz", W/"def"')]
        query_string_weak_etags = {"weak_etags": True}
        for method in all_methods:
            res = method(
                "/objects/1", headers=headers, query_string=query_string_weak_etags
        ...
        res = client.get("/objects/1", headers={"If-Modified-Since": last_modified})
        assert res.cache_control.no_cache
        assert res.status_code == 304
